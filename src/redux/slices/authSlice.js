  ...
  async (profileData, { rejectWithValue }) => {
    try {
      ...
      if (response?.tokens?.accessToken && response?.tokens?.refreshToken) {
        ...
        console.log('PROFILE TOKENS SAVED');
      }
      return response;
    } catch (error) {
      console.log(
  ...
  extraReducers: builder => {
    ...
    builder
      ...
      .addCase(verifyOtp.fulfilled, (state, action) => {
        ...
        state.isNewUser = data?.isNewUser ?? false;
        state.profileCompleted =
          data?.profileCompleted ?? data?.user?.profileCompleted ?? false;
...
export const { setEmail, clearAuthError, resetOtpState, clearAuthState } =
  authSlice.actions;
